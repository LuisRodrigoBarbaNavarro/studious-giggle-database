...
*/
SELECT id_tarjeta, id_institucion, nombre_inst, id_cliente, 
concat(nombre_cliente, " ", apellido_paterno, " ", apellido_materno) AS nombre, puntos
FROM cliente
...
*/
SELECT id_cliente, concat(nombre_cliente, " ", apellido_paterno, " ", apellido_materno) AS nombre, 
...
*/
SELECT 
table_1.id_cliente, table_1.id_institucion, 
table_1.nombre_cliente, table_1.apellido_paterno, 
table_1.apellido_materno,
count
(CASE
...
(
	...
	GROUP BY id_cliente, id_tarjeta
) AS table_1
INNER JOIN
(
...
-- a. Un 10% de las compras realizadas de un producto en particular durante un periodo determinado.
SELECT * FROM tarjeta;
...
(
	...
	id_cliente,
	id_tarjeta,
	(puntos + (total_gastado * 0.1)) AS puntos_aumentados
	FROM
	(
		SELECT
		id_cliente,
...
SET puntos = puntos_aumentados
WHERE t.id_tarjeta = temp.id_tarjeta;

-- b. Un 5% del saldo actual de los clientes de la universidad que más ha consumido en dinero en un periodo en particular.
SELECT * FROM tarjeta;
			...
			(
				...
				INNER JOIN factura USING (id_cliente)
					INNER JOIN detalle USING (id_factura)
						INNER JOIN inventario_maquina USING (id_maquina , id_producto)
				WHERE fecha_expedicion BETWEEN '20210212' AND '20211229'
				GROUP BY id_institucion) AS table_1
			) AS table_2
	)
) AS temp
SET puntos = (puntos * 1.05)
WHERE t.id_cliente = temp.id_cliente
AND estatus = "Habilitada";

-- c. Un 2% del saldo actual de los clientes a los clientes que han comprado diariamente en un periodo en particular.
SELECT * FROM tarjeta;
...
/*
11. Traslade el saldo de las tarjetas inactivas a las activas de cada cliente. 
(las tarjetas inactivas deben quedar en cero y las activas concentrar todo el saldo que hubiera entre activas e inactivas), 
(considere que cada cliente solo tiene una tarjeta activa).
*/
SELECT * FROM temp_tarjeta;
